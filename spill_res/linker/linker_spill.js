...
function update() {
    ...
    if (keys[38] || keys[32]) {
        ...
        if (!spiller.jumping && spiller.grounded) {
            ...
            spiller.velY = -spiller.speed * 1.4;
        }
    }
    if (keys[39]) {
        // Høyre-pil
    ...
    for (var i = 0; i < mål.length; i++) {
        ctx.rect(mål[i].x, mål[i].y, mål[i].width, mål[i].height);

        var dir = colCheck(spiller, mål[i]);
    ...
    ctx.fill();
    ctx.drawImage(spillerBilde, spiller.x, spiller.y-35);
