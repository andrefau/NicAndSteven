...
function update() {
   ...
   // spiller.velY += gravity;
    ...
    if (spiller.x > 69 && spiller.x < 80 && spiller.y > 150) {
        document.getElementById("start").setAttribute("href", "../hinder/hinder.html");
        $("#start").css("opacity", "100");
    }
    // Level 2
    else if (spiller.x > 230 && spiller.x < 270 && spiller.y < 190) {
        document.getElementById("start").setAttribute("href", "../liste/liste.html");
        $("#start").css("opacity", "100");
    }
    // Level 3
    else if (spiller.x > 230 && spiller.x < 270 && spiller.y > 310) {
        document.getElementById("start").setAttribute("href", "../tiger/tiger.html");
        $("#start").css("opacity", "100");
    }
    // Level 4
    else if (spiller.x > 570 && spiller.x < 610 && spiller.y < 290 && spiller.y > 250) {
        document.getElementById("start").setAttribute("href", "../mismatch/mismatch.html");
        $("#start").css("opacity", "100");
    }
    // Level 5
    else if (spiller.x > 720 && spiller.y > 250 && spiller.y < 290) {
        document.getElementById("start").setAttribute("href", "../linker/linker.html");
        $("#start").css("opacity", "100");
    }
    // Level 6
    else if (spiller.x > 720 && spiller.y < 30) {
        document.getElementById("start").setAttribute("href", "../bur/bur.html");
        $("#start").css("opacity", "100");
    }
    // Level 7
    else if (spiller.x > 540 && spiller.x < 580 && spiller.y < 50) {
        document.getElementById("start").setAttribute("href", "../manus/manus.html");
        $("#start").css("opacity", "100");
    }
    // Level 8
    else if (spiller.x > 380 && spiller.x < 430 && spiller.y < 50) {
        document.getElementById("start").setAttribute("href", "../form/formLevel.html");
        $("#start").css("opacity", "100");
    }
    else {
        $("#start").css("opacity", "0");
